...
export default function Dashboard() {
  ...
  useEffect(() => {
    fetch("http://localhost:5000/api/ideas/analytics")
      .then(res => res.json())
      .then(data => {
              ...
              <div className="space-y-4">
                 {TOP_IDEAS.map((idea, i) => (
                    <div key={i} className="flex items-center justify-between gap-4 p-4 rounded-2xl bg-stone-50 border border-stone-100/50 hover:border-orange-200 transition-all text-stone-900">
                       <div className="flex items-center gap-4">
                          <span className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-black italic shadow-lg shadow-stone-200 ${
                            i === 0 ? 'bg-stone-900 text-white' : 'bg-white text-stone-400'
                          }`}>
                            #{idea.rank}
                          </span>
                          <div>
